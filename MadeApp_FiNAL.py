...
if page == '홈':
    st.title("🧠 생활 관리 프로그램")
    st.info("**왼쪽 사이드바**에서 원하는 기능을 선택해주세요.")
...
if page == '수면 리듬 관리':
    ...
    if st.session_state.current_view == 'sleep_results':
        ...
        st.session_state.sleep_mode_input = mode
        if mode == "생체 리듬 생성":
...
if page == '식단 조절':
    ...
    if st.session_state.current_view == 'diet_results':
        ...
        if days_left > 0:
            st.write(f"**{date.today().strftime('%Y년 %m월 %d일')} ~ {(date.today() + timedelta(days=days_left)).strftime('%Y년 %m월 %d일')}**")
            st.markdown(f"""
                - **총 권장 칼로리:** {target_kcal:.0f} kcal
                - **아침 칼로리:** {breakfast:.0f} kcal
                - **점심 칼로리:** {lunch:.0f} kcal
                - **저녁 칼로리:** {dinner:.0f} kcal
            """)
        else:
